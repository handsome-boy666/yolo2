...
### 1.1 锚框
YOLOv2 借鉴了 Faster R-CNN 的锚框思想（预先设定预测框的大小），同时通过**维度聚类**优化锚框设计，解决了手动设计锚框的主观性问题。

#### （1）锚框
锚框是预设在特征图每个网格上的固定尺寸边框，每个网格不再直接预测框的绝对尺寸，而是预测锚框的偏移量和缩放因子。
    * 每个网格预设 k 个锚框（YOLOv2 默认为 5 个）；
    * 模型输出不再是直接的框坐标，而是「锚框的 x/y 偏移量」和「w/h 缩放因子」；
    * 最终框坐标通过公式解码：
...
这种设计将**直接回归框尺寸**转化为**回归锚框的相对偏移**，大幅降低了回归难度，提升了框预测的稳定性。
#### （2）维度聚类：找锚框的方法
