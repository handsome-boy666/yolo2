...
def plot_boxes(img, boxes, class_names, color=None):
    # img: cv2 image
    # boxes: [x1, y1, x2, y2, cls_conf, cls_pred] or similar
    ...
    for i in range(len(boxes)):
        ...
        x2 = int(box[2])
        y2 = int(box[3])
        # Check if box has enough elements
        if len(box) >= 6:
            cls_conf = box[4]
            cls_id = int(box[5])
        ...
        if color:
            ...
            rgb = (255, 0, 0) # Blue in RGB, but cv2 uses BGR. 
            # If input is RGB, we use RGB color. Assuming RGB for now or BGR handled by caller.
        
        cv2.rectangle(img, (x1, y1), (x2, y2), rgb, 2)
        
        t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 1, 1)[0]
        c2 = x1 + t_size[0] + 3, y1 + t_size[1] + 4
        cv2.rectangle(img, (x1, y1), c2, rgb, -1)
